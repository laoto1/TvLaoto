        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                view?.evaluateJavascript(
                    ...
                    "               if (vids[i].paused) { " +
                    "                   vids[i].muted = true; " +
                    "                   vids[i].play().then(function() { console.log('Playing!'); }).catch(function(e) { console.log('Play error: ' + e); }); " +
                    "               } " +
                    "           } " +
                    "       } else { " +
                    ...
                    "           for (var i = 0; i < buttons.length; i++) { buttons[i].click(); console.log('Clicked button'); } " +
                    "       } " +
                    "   }, 1000); " +
                    "})();", null
                )
...
with open('app/src/main/java/com/tvlaoto/network/StreamSniffer.kt', 'w', encoding='utf-8') as f:
    f.write(content)
print("Fixed StreamSniffer.kt correctly without regex")
